...
class TactRecord:
    ...
    def close_interval_instances(self, interval: Union[str, KBInterval, IntervalInstance], tact: int) -> IntervalInstance:
        ...
        if existing_instance.open_tact >= tact:
            raise ValueError(f"Supposed close tact must be greater than open tact {existing_instance.open_tact}, but got {tact}")
    ...
    def __dict__(self):
        return {
            ...
            'opened_intervals': [
                {
                    'interval': instance.interval.id, 
                    'open_tact': instance.open_tact, 
                    'close_tact': instance.close_tact
                } 
                for instance in self.opened_interval_instances
            ],
            'events': [
                {
                    ...
                    'occurance_tact': instance.occurance_tact
                }
                for instance in self.event_instances
            ],
        }
...
class Timeline:
    ...
    def create_event_instance(self, tact: int, event: KBEvent) -> EventInstance:
        tact_record = self.get_or_create_tact_record(tact)
        instance = tact_record.create_event_instance(event)
        return instance
    ...
    def __dict__(self):
        return {
            'tacts': [
                tact.__dict__ for tact in self.sorted_tact_list    
            ]
        }